...
class TestZoom(TestSettingsBase):
    ...
    def setUp(self):
        ...
        # URLs
        self.single_recording_url = (
            "https://api.zoom.us/v2/meetings/some-meeting-id/recordings/rid"
        )
        self.single_meeting_recording_info_url = (
            "https://api.zoom.us/v2/meetings/some-meeting-id/" "recordings"
        )
        self.single_recording_download = (
            "https://mindsai.zoom.us/recording/share/random-uid"
        )
        self.zak_token_url = (
            f"https://api.zoom.us/v2/users/{self.zoom_object.username}/token"
        )
    ...
    def test_generate_jwt_valid_token(self):
        token = jwt.encode(
            {"iss": self.zoom_object.key, "exp": int(time.time() + 1800)},
    ...
    def test_downloading_file(self):
        responses.add(
            ...
            self.zak_token_url,
            status=404,
            json=self.zak_token_return_payload,
        )
        responses.add(
            responses.GET, self.single_recording_download, status=200, stream=True
        )
    ...
    def test_filesystem_errors_file_pull(self):
        ...
        self.api.download_recording = MagicMock(
            side_effect=OSError("Could not write file!")
        )
        self.assertEqual(
            self.api.pull_file_from_zoom("some-meeting-id", True),
            {"success": False, "date": None, "filename": None},
