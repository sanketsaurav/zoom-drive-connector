...
class ZoomConfig(APIConfigBase):
    ...
    """
        ...
        if not all(
            k in self.settings_dict
            for k in ("key", "secret", "username", "delete", "meetings")
        ):
            return False

        for meeting in self.settings_dict["meetings"]:
            if not all(
                k in meeting for k in ("id", "folder_id", "name", "slack_channel")
            ):
                return False
    ...
    """
        try:
            ...
            if hasattr(ye, "problem_mark"):
                log.log(
                    logging.INFO,
                    f"Error at position ({ye.problem_mark.line + 1}, "
    ...
    """
        ...
        for value in self.configuration_dict.values():
            if not value.validate():
                raise RuntimeError(
                    f"Configuration for section {value.__class__} failed validation step."
                )
